...
class WebScraping():
    ...
    def execute(self, categoriasLista=[], tipoUrl=''):
        try:
            categoriasOK, categoriasNOEX = asyncio.run(self.extraer_libros())
            return (categoriasOK, categoriasNOEX)
        except KeyboardInterrupt:
            print("*** Stop web scraping ***")
            exit(1)

    async def extraer_libros(self):
        categoriasOK = []
        categoriasNOEX = []
        async with async_playwright() as p:
            ...
            await context.storage_state(path="./estado_sesion.json")
            print("...2222!")
            ...
            try:
                ...
                print(catIdPadre)
                deeplinkUrl = (
                    "https://api.test.tottus.cl/categories?name="
                    + categoriaClear.replace(',','')
                    + "&id=" + catIdHijo
                    + "&landingtonewPLP=true&defaultSelectParentId="
                    + catIdPadre[idPadrePosInicial:idPadrePosFinal]
                    + "&defaultSortBy=Recomendados"
                )
...
if __name__ == "__main__":
    ws = WebScraping("chromium")
    categoriasOK, categoriasNOEX = ws.execute()
    print("Categorias OK:", categoriasOK)
    print("Categorias NOEX:", categoriasNOEX)
...
class WebScraping():
    ...
    def execute(self, categoriasLista=[], tipoUrl=''):
        try:
            categoriasOK, categoriasNOEX = asyncio.run(self.extraer_libros(categoriasLista))
            return (categoriasOK, categoriasNOEX)
        except KeyboardInterrupt:
            print("*** Stop web scraping ***")
            exit(1)

    async def extraer_libros(self, categoriasLista):
        #print("111")
        #print(categoriasLista)
        #print("2222")
        categoriasOK = []
        categoriasNOEX = []
        async with async_playwright() as p:
...
#Actual           : 2025-07-31  ---> 31 de Julio del 2025
...
class WebScraping():
    ...
    async def extraer_libros(self, categoriasLista):
        ...
        async with async_playwright() as p:
            ...
            browser = await p.chromium.launch(headless=False)
            #Cargo en el navegador la sesion guardada de la plataforma a ingresar
            context = await browser.new_context(storage_state="estado_sesion.json")
            #Abro una nueva pestaña en el navegador
            page = await context.new_page()

            #Ingreso a la web de la plataforma
            await page.goto("https://admin.fazil.services/application/catalog/taxonomy/categories")
            await asyncio.sleep(5)
            await page.locator('.nice-button').click()
            await context.storage_state(path="./estado_sesion.json")
